           ...
           (λ (r row)
             (map (λ (c n)
                    (if (natural? n)
                        (sub1 n)
                        (let ((v (string->symbol
                                  (format "r~a~a" (add1 r) (add1 c)))))
                          (set! vars (append vars `(,v)))
                          v)))
                  (range sz)
                  row))
           (range sz)
...
(define (gen-run sz board)
  (match-let (((cons board^ vars) (gen-vars sz board)))
    `((run 1 ,(map (λ (var)
                     `(,var : ,(string->symbol (format "n~a" sz))))
                   vars)
           (,(string->symbol (format "sudoku_~ax~a" sz sz))
            .
            ,(append* board^))))))

;;; The following examples are copied from https://sudoku.com/.
(define sudoku4-easy
  ;;; Running a goal
  (append
   sudoku4
